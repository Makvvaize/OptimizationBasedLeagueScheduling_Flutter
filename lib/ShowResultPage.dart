...
class _ShowResultPageState extends State<ShowResultPage> {
  ...
  Future<double> fetchTravelDistance(List<String> homeTeamID, List<String> awayTeamID) async {
    // Fetch CITY info
    ...
    final travelDistance = calculateDistance(homeLat, homeLng, awayLat, awayLng);
  ...
  Future<List<Map<String, dynamic>>> fetchFixtures() async {
    ...
    for (int i = 0; i < result1.length; i++) {
      final travelDistance = await fetchTravelDistance([result2[i]], [result3[i]]);
      // Check if the team has a break in this week
      if (breakResults.any((element) =>
      element['TEAM_ID'] == result2[i] && element['WEEK_INFO'] == result4[i] &&
      ...
      else if (breakResults.any((element) => element['TEAM_ID'] == result2[i] &&
          element['WEEK_INFO'] == result4[i])) {
        fixtures.add({
          'matchId': result1[i],
          'homeTeamId': result2[i],
      ...
      else if (breakResults.any((element) => element['TEAM_ID'] == result3[i] &&
          element['WEEK_INFO'] == result4[i])) {
        fixtures.add({
          'matchId': result1[i],
          'homeTeamId': result2[i],
  ...
  Future<String> getTeamName(String teamID) async {
    String result = await database.sendQuery(
        "SELECT TEAM_NAME FROM TEAM_INFO WHERE TEAM_ID = $teamID");
    return result;
  }

  List<Map<String, dynamic>> getFixturesForWeek(String week) {
    return fixtures.where((fixture) =>
    fixture['matchWeek'].toString() == week.replaceAll('Week ', '')).toList();
  }

  List<List<Map<String, dynamic>>> groupFixturesByWeek() {
    List<List<Map<String, dynamic>>> fixturesByWeek = List.generate(
        weekCount, (_) => []);
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Scaffold(
              ...
              Row(
                ...
                children: [
                  ...
                  Tooltip(
                    ...
                    child: IconButton(
                      onPressed: () async {
                        List<Map<String, dynamic>> breakCounts = await fetchBreakCounts();
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => BreakCountsPage(breakCounts: breakCounts),
                          ),
                        );
                      },
                    ...
                    child: IconButton(
                      onPressed: () async {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => SwapWeeksPage(weekList: weekList, teamNames: teamNames, leagueName: leagueName, teamCount: teamCount),
                          ),
                        );
                      },
                        ...
                        title: Row(
                          ...
                          children: [
                            Text(
                              'Match ${index + 1}',
                              style: TextStyle(
                                  fontSize: 18, color: Colors.teal[300]),
                            ),
                          ],
                        ),
                        subtitle: Row(
                        ...
                        return SingleChildScrollView(
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                const Text(
                                  "1 - Teams written in red have a break in this week.\n2 - If you made swap between weeks, you may see conflicts on breaks \n3 - Pressing back button after swap, will forward you to last fixture before swap. \n4 - Back to Main Page will forward you directly to main page , If you want to save the league please download it as an excel from top right",
                                ...
                                ElevatedButton(
                                  ...
                                  child: Text('Exit', style: TextStyle(color: Colors.deepPurple[100])),
                                  onPressed: () {
          ...
          floatingActionButtonLocation: FloatingActionButtonLocation.endFloat, // Added for FloatingActionButton
          floatingActionButton: FloatingActionButton.extended(
            ...
            label: Text('Back to Main Page' , style: TextStyle(color: Colors.deepPurple[100]),),
              ...
              return Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(
                      'Week ${weekIndex + 1}',
                      style: TextStyle(fontSize: 24, color: Colors.deepPurple[100], fontWeight: FontWeight.bold),
                    ),
                  ),
                  DataTable(
                    headingRowHeight: 40,
                    ...
                    columns: [
                      DataColumn(
                        label: Text(
                          'Home Team',
                          style: TextStyle(fontSize: 18, color: Colors.red[500], fontWeight: FontWeight.bold),
                        ),
                      ),
                      DataColumn(
                        label: Text(
                    ...
                    rows: fixturesByWeek[weekIndex].map((fixture) {
                      return DataRow(cells: [
                        DataCell(
                          Text(
                            fixture['homeTeamName'] ?? '',
                            style: TextStyle(fontSize: 18, color: Colors.deepPurple[100]),
                          ),
                        ),
                        DataCell(
                          Text(
...
class TeamSchedulePage extends StatelessWidget {
  ...
  TeamSchedulePage({required this.teamNames, required this.selectedTeam, required this.fixturesByWeek});
    ...
    for (var weekFixtures in fixturesByWeek) {
      for (var fixture in weekFixtures) {
        if (fixture.containsValue(selectedTeam)) {
          schedule.add('${fixture['homeTeamName']} vs ${fixture['awayTeamName']}');
        }
      }
    }
      ...
      child: Container(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            iconTheme: IconThemeData(color: Colors.deepPurple[100]),
            backgroundColor: Colors.transparent,
            title: Text('Schedule for $selectedTeam', style: TextStyle(fontSize: 20, color: Colors.deepPurple[100])),
          ),
          body: ListView.builder(
      ...
      child: Container(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            iconTheme: IconThemeData(color: Colors.deepPurple[100]),
            backgroundColor: Colors.transparent,
            title: Text('Select Team', style: TextStyle(fontSize: 20, color: Colors.deepPurple[100])),
          ),
          body: ListView.builder(
  ...
  Widget build(BuildContext context) {
    ...
    for (var fixture in fixtures) {
      ...
      if (totalTravelDistances.containsKey(awayTeamName)) {
        totalTravelDistances[awayTeamName] = totalTravelDistances[awayTeamName]! + travelDistance;
      } else {
    ...
    List<MapEntry<String, double>> sortedEntries = totalTravelDistances.entries.toList()
      ...
      child: Container(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            iconTheme: IconThemeData(color: Colors.deepPurple[100]),
            backgroundColor: Colors.transparent,
            title: Text('Travel Distances', style: TextStyle(color: Colors.deepPurple[100])),
          ),
          body: ListView.builder(
          ...
          body: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return ListTile(
                ...
                subtitle: Center( // Wrap the FutureBuilder with Center widget
...
class SwapWeeksPage extends StatefulWidget {
  ...
  SwapWeeksPage({required this.weekList, required this.teamNames, required this.teamCount, required this.leagueName});
...
class _SwapWeeksPageState extends State<SwapWeeksPage> {
                      ...
                      children: [
                        Row (
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child:Text('SWAP',style: TextStyle(color: Colors.deepPurple[100], fontWeight: FontWeight.bold , fontSize: 24)),
                            ),
                            SizedBox(
                              width: 140,
                              height: 55,
                        ...
                        Container(
                          ...
                          child:ElevatedButton(
                            onPressed: () {
                              swappedWeek1 = int.parse(dropdownValue.replaceAll('Week ', ''));
                              swappedWeek2 = int.parse(dropdownValue2.replaceAll('Week ', ''));
                              print("Swapped week: $swappedWeek1 with week: $swappedWeek2");
                              if (_swapWeeks())  {
                            ...
                            child: Text('Swap Weeks' ,style: TextStyle(fontSize: 18, color: Colors.deepPurple[100]),),
  ...
  bool _swapWeeks() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Weeks $swappedWeek1 and $swappedWeek2 swapped successfully'),
        backgroundColor: Colors.green,
      ),
    );
    return true;
